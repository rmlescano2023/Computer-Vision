...
for i in range(N_levels):
    ...
    # Manually resize the images to reduce dimensions by half
    G1 = cv2.resize(G1, (G1.shape[1] // 2, G1.shape[0] // 2))
    G2 = cv2.resize(G2, (G2.shape[1] // 2, G2.shape[0] // 2))
...
for i in range(N_levels - 1, 0, -1):
    ...
    upscaled_A = cv2.resize(lstA_G[i], (lstA_G[i-1].shape[1], lstA_G[i-1].shape[0]))
    upscaled_B = cv2.resize(lstB_G[i], (lstB_G[i-1].shape[1], lstB_G[i-1].shape[0]))
    
    # Construct Laplacian images
    L1 = lstA_G[i-1] - upscaled_A
    lstA_L.append(L1)
    
    L2 = lstB_G[i-1] - upscaled_B
    lstB_L.append(L2)
...
LS = []
for la, lb in zip(lstA_L, lstB_L):      # zip() function can basically iterate throough multiple lists as its parameters, and then returns a tuple
...
# Start with the bottom of the Laplacian Pyramid
...
for i in range(1, N_levels):
    ...
    upscaled_blend = cv2.resize(blend_with_invisible_edge, (LS[i].shape[1], LS[i].shape[0]))
    
    # Add the upscaled blended image and the Laplacian image at the current level
    blend_with_invisible_edge = upscaled_blend + LS[i]
